...
bool isNum(const string& str)//判断一个字符串是不是全是数字
{
...
vector<string> readCfg(const string& path)//从指定路径读取配置文件
{
	vector<string>ret;
	ifstream fin(path);
	if (!fin.is_open())
...
vector<pair<string, string>> parseCfg(vector<string> cfgLines)
{
	...
	for (int i = 0; i < cfgLines.size();i++)
	{
		...
		if (equalCnt != 1)//等号数量必须为1，否则不合法
			continue;
		string str1 = strip(line.substr(0, equalIndex));
		//相当于python str1[0:equalIndex].strip()
		string str2 = strip(line.substr(equalIndex+1, line.size()- equalIndex));
...
void writeCfg(const string& path)//将当前设置写入cfg
{
	ofstream fout{ path };
	if (!fout.is_open())
		return;
	//打不开就算了
	fout << "scale=" << note::getScale() << endl;
	fout << "volume=" << volume::getVolume100() << endl;
	fout << "delay=" << play::getDelay() << endl;
	fout << "instrument=" << instrument::getIns() << endl;
	fout.close();
}
...
{
	vector<pair<string, string>> parsedCfg = parseCfg(readCfg(path));
	//从readCfg()读，经parseCfg()解析
	for (pair<string, string> line : parsedCfg)
	{
		if (line.first == "scale")
		{
		...
		if (line.first == "volume")
		{
			if (isLegalNum(line.second, 0, 100))
				volume::setVolume(atoi(line.second.c_str()));
		}
		if (line.first == "delay")
		{
			if (isLegalNum(line.second, 0, 5000))
				play::setDelay(atoi(line.second.c_str()));
		}
